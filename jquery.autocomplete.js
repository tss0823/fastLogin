...
$.Autocompleter = function(input, options) {
	...
	var $inputPwd = $("#"+options.inputPwd);   //add by tangshengshan
    if($inputPwd.length == 0){
        $inputPwd = $("input[name='"+options.inputPwd+"']")
    }
	...
	function selectCurrent() {
		...
		$input.val(v);
        if($inputPwd){   //add pwd modify by tangshengshan
            var pwdVal = options.dataMap[v];
            $inputPwd.val(pwdVal);
        }
		hideResultsNow();
